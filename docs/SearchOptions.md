...
|    指令     |  建议形式   | 默认值 |                     可用                      |       作用       |
| :---------: | :---------: | :----: | :-------------------------------------------: | :--------------: |
| `$realtime` | `$realtime` |  0/1   | both: `v0.0.4-beta` <sup>[\[3\]](#其他)</sup> | 是否进行实时搜索 |
...
3. `$realtime` 的  TypeScript (cfw/deno) 实现暂不支持“延迟”资源的简繁体转换。  
   即关键词为简中时，同名的繁中“延迟”资源不会出现在搜索结果中，反之亦然。  
   理由是暂时未能找到或实现一个轻量且全面的 JavaScript 简繁转换函数。而 go/rust 实现没有此问题。

   注意，由 dmhy 本身返回的搜索结果始终是忽略简繁体字差异的，只是对于由 `$realtime` 找到的资源，各语言的实现有所区别。

### $page

|  指令   | 建议形式  | 默认值 |        可用         |         作用          |
| :-----: | :-------: | :----: | :-----------------: | :-------------------: |
| `$page` | `$page:X` |  1/1   | both: `v0.0.4-beta` | 获取搜索结果的第 X 页 |
...
|   指令   |  建议形式  | 默认值 |        可用         |            作用            |
| :------: | :--------: | :----: | :-----------------: | :------------------------: |
| `$limit` | `$limit:X` | 200/80 | both: `v0.0.4-beta` | 限制搜索结果的数量上限为 X |
